...
<body>
    ...
    @php
        $totalSiswa = $recap->count();
        $avgKehadiran = $hariEfektif > 0 && $totalSiswa > 0
            ? round($recap->sum(fn($r) => ($r['hadir'] ?? 0)) / ($totalSiswa * $hariEfektif) * 100, 1)
            : 0;
        $siswaDibawah75 = $hariEfektif > 0
            ? $recap->filter(fn($r) => (($r['hadir'] ?? 0) / $hariEfektif * 100) < 75)->count()
            : 0;
    @endphp

    <table class="summary">
        <tr>
            <td class="label">Hari Efektif Sekolah</td>
        ...
        @if($siswaDibawah75 > 0)
        <tr>
            <td class="label">Siswa dengan Kehadiran &lt; 75%</td>
            <td class="value-danger">{{ $siswaDibawah75 }} siswa</td>
        </tr>
        @endif
    </table>
        ...
        <tbody>
            @forelse($recap as $i => $row)
                @php
                    ...
                    $pctHadir = $hariEfektif > 0 ? round(($row['hadir'] ?? 0) / $hariEfektif * 100, 1) : 0;
                    $isLow = $pctHadir < 75;
                @endphp
    ...
    @if($siswaDibawah75 > 0)
        <div class="insight-box">
            <strong>Perhatian:</strong> {{ $siswaDibawah75 }} siswa memiliki tingkat kehadiran di bawah 75% (ditandai merah).
            Disarankan untuk menghubungi orang tua siswa terkait agar kehadiran dapat ditingkatkan.
        </div>
    @endif
