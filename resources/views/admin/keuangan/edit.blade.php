    ...
    <div class="max-w-md bg-white rounded-xl shadow-ich-card p-6">
        ...
        <form method="POST" action="{{ route('admin.keuangan.update', $keuangan) }}" class="space-y-4">
            ...
            <div>
                ...
                <select name="status"
                    class="w-full h-[46px] px-3.5 bg-white border-2 border-ich-teal rounded-ich-lg font-sans text-sm focus:outline-none">
                    <option value="unpaid" {{ $keuangan->status === 'unpaid' ? 'selected' : '' }}>Belum Bayar
                    </option>
                    <option value="paid" {{ $keuangan->status === 'paid' ? 'selected' : '' }}>Lunas</option>
                </select>
            </div>

            <div class="flex gap-3 pt-2">
                <button type="submit" class="px-6 py-2.5 bg-ich-green text-white font-ui font-bold text-sm
                               rounded-ich-lg shadow-ich-btn hover:bg-ich-green-dark transition-colors">
                    Perbarui
                </button>
                <a href="{{ route('admin.keuangan.index') }}" class="px-6 py-2.5 bg-white border border-ich-line text-ich-ink-600 font-ui font-bold text-sm
