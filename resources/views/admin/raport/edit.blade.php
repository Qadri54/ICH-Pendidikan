...
<x-main-layout title="Edit Raport">
    ...
    <div x-data="{
        ...
        <div x-show="tab === 'narasi'" x-cloak>
            <form method="POST"
                  action="{{ route('admin.raport.narrative', $raport->report_card_id) }}?tab=narasi">
                @csrf
                <div class="space-y-4">
                    @foreach($defaultNarratives as $i => $default)
                        ...
                        <div class="bg-white rounded-xl shadow-ich-card p-5">
                            ...
                            <textarea name="narratives[{{ $i }}][isi_naratif]"
                                      rows="4"
        ...
        <div x-show="tab === 'checklist'" x-cloak>
            <form method="POST"
                  action="{{ route('admin.raport.checklist', $raport->report_card_id) }}?tab=checklist">
                @csrf
                @php $idx = 0; @endphp
                ...
                @empty
                    <div class="bg-white rounded-xl shadow-ich-card p-10 text-center">
                        <p class="font-sans text-ich-ink-400">
                            Belum ada kategori checklist. Tambahkan melalui menu master data.
                        </p>
                    </div>
                @endforelse
        ...
        <div x-show="tab === 'fisik'" x-cloak>
            <form method="POST"
                  action="{{ route('admin.raport.physical', $raport->report_card_id) }}?tab=fisik">
                @csrf
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="number" name="tinggi_badan" step="0.1"
                                       value="{{ old('tinggi_badan', $physical?->tinggi_badan) }}"
                            ...
                            <div>
                                ...
                                <input type="number" name="berat_badan" step="0.1"
                                       value="{{ old('berat_badan', $physical?->berat_badan) }}"
                                       placeholder="misal: 18.5"
                                       class="w-full h-10 px-3 bg-white border-2 border-ich-line rounded-ich-lg
                                              font-sans text-sm focus:outline-none focus:border-ich-teal">
                            </div>
                            <div>
                                ...
                                <input type="number" name="lingkar_kepala" step="0.1"
                                       value="{{ old('lingkar_kepala', $physical?->lingkar_kepala) }}"
                                       placeholder="misal: 50.0"
                                       class="w-full h-10 px-3 bg-white border-2 border-ich-line rounded-ich-lg
                                              font-sans text-sm focus:outline-none focus:border-ich-teal">
                            </div>
                            <div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="pendengaran" required
                                       ...
                                       class="w-full h-10 px-3 bg-white border-2 border-ich-line rounded-ich-lg
                                              font-sans text-sm focus:outline-none focus:border-ich-teal
                                              @error('pendengaran') border-ich-error @enderror">
                            </div>
                            <div>
                                <label class="block font-ui font-bold text-xs text-ich-ink-600 mb-1.5">
                                    Penglihatan
                                ...
                                <input type="text" name="penglihatan" required
                                       ...
                                       class="w-full h-10 px-3 bg-white border-2 border-ich-line rounded-ich-lg
                                              font-sans text-sm focus:outline-none focus:border-ich-teal
                                              @error('penglihatan') border-ich-error @enderror">
                            </div>
                            <div>
                                <label class="block font-ui font-bold text-xs text-ich-ink-600 mb-1.5">
                                    Catatan Tambahan
