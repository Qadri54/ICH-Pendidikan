...
<x-main-layout title="Pengaturan">
    ...
    <div x-data="{ tab: 'pendaftaran' }" class="max-w-4xl">
        ...
        {{-- Tab Navigation --}}
        <div class="flex overflow-x-auto no-scrollbar gap-2 bg-white rounded-xl shadow-ich-card p-2 mb-5">
            <button @click="tab = 'pendaftaran'" type="button"
                    ...
                    class="flex-1 whitespace-nowrap shrink-0 px-4 min-w-fit flex items-center justify-center gap-2 py-2.5 rounded-lg font-ui font-bold text-sm transition-all">
            ...
            <button @click="tab = 'biaya'" type="button"
                    :class="tab === 'biaya' ? 'bg-ich-teal text-white shadow-sm' : 'text-ich-ink-500 hover:bg-ich-surface'"
                    class="flex-1 whitespace-nowrap shrink-0 px-4 min-w-fit flex items-center justify-center gap-2 py-2.5 rounded-lg font-ui font-bold text-sm transition-all">
            ...
            <div class="bg-white rounded-xl shadow-ich-card p-6">
                <div class="flex items-center gap-3 mb-6">
                    <div class="w-9 h-9 rounded-lg bg-ich-teal/10 flex items-center justify-center">
                        <svg class="w-5 h-5 text-ich-teal" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                    <div>
                        <h2 class="font-display font-bold text-ich-ink-900">Tarif SPP & Pendaftaran</h2>
                        <p class="text-xs text-ich-ink-400 font-sans">Atur biaya bulanan (SPP) dan biaya pendaftaran siswa baru</p>
                    </div>
                </div>

                <form method="POST" action="{{ route('admin.pengaturan.fee.update') }}">
                    @csrf
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-6">
                        <div>
                            <label class="block text-sm font-ui font-semibold text-ich-ink-700 mb-1.5">Biaya SPP Bulanan</label>
                            <div class="relative">
                                <span class="absolute left-3 top-2.5 text-sm font-ui text-ich-ink-400">Rp</span>
                                <input type="number" name="spp_fee" value="{{ old('spp_fee', $feeSetting?->spp_fee ?? 300000) }}" required
                                       class="w-full pl-9 pr-3.5 py-2.5 bg-ich-surface border-2 border-ich-line rounded-ich-lg font-sans text-sm focus:outline-none focus:border-ich-teal">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-ui font-semibold text-ich-ink-700 mb-1.5">Biaya Pendaftaran</label>
                            <div class="relative">
                                <span class="absolute left-3 top-2.5 text-sm font-ui text-ich-ink-400">Rp</span>
                                <input type="number" name="registration_fee" value="{{ old('registration_fee', $feeSetting?->registration_fee ?? 3000000) }}" required
                                       class="w-full pl-9 pr-3.5 py-2.5 bg-ich-surface border-2 border-ich-line rounded-ich-lg font-sans text-sm focus:outline-none focus:border-ich-teal">
                            </div>
                        </div>
                    </div>
            ...
            <div class="bg-white rounded-xl shadow-ich-card p-6" x-data="{
                ...
                @else
                    ...
                    <div class="border-t border-ich-line pt-6 mt-6">
                        ...
                        <form method="POST" action="{{ route('admin.pengaturan.whatsapp.test') }}" class="flex flex-col sm:flex-row gap-3">
                            @csrf
                            <input type="text" name="test_phone"
                                   ...
                                   class="flex-1 h-[46px] px-3.5 bg-white border-2 border-ich-line rounded-ich-lg
                                          font-sans text-sm focus:outline-none focus:border-ich-teal">
                            <button type="submit"
                                    class="w-full sm:w-auto px-5 py-2.5 bg-ich-teal text-white font-ui font-bold text-sm
