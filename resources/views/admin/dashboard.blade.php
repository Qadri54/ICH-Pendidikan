<x-main-layout title="Dashboard">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-xl shadow-ich-card p-5 flex items-start gap-4">
            <div class="w-11 h-11 rounded-xl bg-[#D1FAE5] flex items-center justify-center flex-shrink-0">
                <svg class="w-5 h-5 text-[#009966]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            </div>
            <div>
                <div class="text-ich-ink-400 text-xs font-sans mb-0.5">Tagihan Lunas</div>
                <div class="text-2xl font-display font-bold text-[#009966]">{{ $stats['tagihan_lunas'] }}</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
        {{-- Revenue highlight --}}
        <div class="lg:col-span-2 bg-gradient-to-br from-ich-green to-[#00785A] rounded-xl shadow-ich-card p-6 relative overflow-hidden">
        ...
        <div class="space-y-4">
            @if($stats['pending_daftar'] > 0)
                <a href="{{ route('admin.pendaftaran.index') }}" class="block bg-white rounded-xl shadow-ich-card p-5 hover:shadow-md transition-shadow no-underline group">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-[#FEF5DC] flex items-center justify-center flex-shrink-0">
                            <svg class="w-5 h-5 text-[#E09F17]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z"/></svg>
                        </div>
                        <div>
                ...
                <a href="{{ route('admin.raport.index') }}" class="block bg-white rounded-xl shadow-ich-card p-5 hover:shadow-md transition-shadow no-underline group">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-[#EDE9FE] flex items-center justify-center flex-shrink-0">
                            <svg class="w-5 h-5 text-[#8B5CF6]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                        </div>
                        <div>
            ...
            @if($stats['pending_daftar'] == 0 && $stats['pending_raport'] == 0)
                <div class="bg-white rounded-xl shadow-ich-card p-5">
                    <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-xl bg-[#D1FAE5] flex items-center justify-center flex-shrink-0">
                            <svg class="w-5 h-5 text-[#009966]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                        </div>
                        <div>
    ...
    <div class="mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            ...
            $menus = [
                ...
                ['label' => 'Pendaftaran',    'desc' => 'PPDB online',             'icon' => 'clipboard', 'route' => 'admin.pendaftaran.index',  'color' => 'bg-[#DBEAFE] text-[#3B82F6]'],
                ['label' => 'Laporan',        'desc' => 'Ringkasan & export',      'icon' => 'document',  'route' => 'admin.laporan.index',      'color' => 'bg-[#FFF7ED] text-[#F97316]'],
